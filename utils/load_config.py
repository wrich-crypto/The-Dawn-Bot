...
def get_proxies() -> List[Proxy]:
    try:
        proxies_file = os.path.join(CONFIG_DATA_PATH, "proxies.txt")
        logger.debug(f"Attempting to read proxies from: {proxies_file}")
        
        if not os.path.exists(proxies_file):
            logger.warning(f"Proxies file not found: {proxies_file}")
            return []
        
        proxies = read_file(proxies_file, check_empty=False)
        
        logger.debug(f"Read {len(proxies)} proxies")
        return [Proxy.from_str(line) for line in proxies] if proxies else []
    except Exception as exc:
        logger.error(f"Failed to parse proxies: {exc}")
        return []
